...
%
% INPUTS:
%    model:                 COBRA model structure (input genome-scale metabolic model).
%
...
%
% OPTIONAL INPUTS:
%    condition:             Row vector specifying the condition indices corresponding to the 
%                           columns in geneexpressionTable.
%                           Examples:
%                               1:3       → conditions 1 to 3
%                               [1:5,7:8] → conditions 1 to 8 excluding condition 6
%                           Default: all conditions.
%
%    normalizemethod:       Method for normalizing gene expression data for each gene (row) to 
%                           relative values. Options:
...
%    [modelIGM, solIGM] = IGM_function(model, uptakeRatesTable, geneexpressionTable, [1:3], 'mean')
%
% Authors: 
...
% filename1 = 'IGM1/igm_fluxcond_carbecoli_NN.csv';
